 ...
 * 
 * [CHEN HAOTING] Modified from the original crawler4j.CrawlController in the library
 * 							& crawler-tripadvisor/src/it/thecrawlers/crawler/CrawlControllerOwn.java from "https://github.com/dangrasso/crawler-tripadvisor"
 * @author CHT
 *
 */
public class CrawlControllerExtension extends CrawlController {
...
//static final Logger logger = LoggerFactory.getLogger(CrawlController.class);
  ...
  public CrawlControllerExtension(CrawlConfig config, PageFetcher pageFetcher, RobotstxtServer robotstxtServer)
      throws Exception {
   ...
   */
  public <T extends WebCrawler> void start(final Class<T> _c, final int numberOfCrawlers, CrawlHandler handler) {
   ...
   * @param <T> Your class extending WebCrawler
   */
  public <T extends WebCrawler> void startNonBlocking(final Class<T> _c, final int numberOfCrawlers, CrawlHandler handler) {
    this.start(_c, numberOfCrawlers, false, handler);
  }

  protected <T extends WebCrawler> void start(final Class<T> _c, final int numberOfCrawlers, boolean isBlocking, CrawlHandler handler) {
    try {
      finished = false;
      ...
      for (int i = 1; i <= numberOfCrawlers; i++) {
    	/** [CHEN HAOTING] Modified: transfer a parameter of an instance of CrawlHandler into the crawler for further operation */
    	Constructor <?>[] constructors=_c.getConstructors();
    	@SuppressWarnings("unchecked")
		T crawler = (T) constructors[0].newInstance(handler);
      ...
      /** Consider again */
      final CrawlHandler finalHandler=handler;		
                ...
                for (int i = 0; i < threads.size(); i++) {
                  ...
                  if (!thread.isAlive()) {
                    if (!shuttingDown) {
                      ...
                      Constructor <?>[] constructors=_c.getConstructors();
                      @SuppressWarnings("unchecked")
					  T crawler = (T) constructors[0].newInstance(finalHandler);
                      //T crawler = _c.newInstance();
                      /** ------------------------------------------- */
